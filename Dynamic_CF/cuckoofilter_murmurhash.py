...
import exceptions
import hashutils_murmurhash


class CuckooFilter_mur(object):
    """
    Cuckoo Filter class.
    ...
    def _get_index(self, item):
        index = hashutils_murmurhash.hash_code(item) % self.capacity
        return index

    def _get_alternate_index(self, index, fingerprint):
        alt_index = (index ^ hashutils_murmurhash.hash_code(fingerprint)) % self.capacity
        return alt_index
    ...
    def insert(self, item):
        ...
        filter is full.
        """
        fingerprint = hashutils_murmurhash.fingerprint(item, self.fingerprint_size)
    ...
    def delete(self, item):
        ...
        """
        fingerprint = hashutils_murmurhash.fingerprint(item, size=self.fingerprint_size)
        i = self._get_index(item)
        j = self._get_alternate_index(i, fingerprint)
