...
sys.path.append(r"C:/Users/fptjy/ArkFilter_experiment/ArkFilter_for_Dynamics")
...
for i in range(len(insert_result)):
    if insert_result[i] == "non" or len(insert_result[i]) == 0:
        ...
        x1 = DAF.capacity * len(DAF.buckets[0].bucket)
        DAF_test_result.append(x1)
...
for i in range(len(delete_result) - len(insert_result)):
    ...
    y1 = DAF.capacity * len(DAF.buckets[0].bucket)
    DAF_test_result.append(y1)
...
##DAF_test_result
# 1. 创建文件对象
f = open('DAF_test_result_jizhi.csv', 'w', encoding='utf-8', newline="")
# 2. 基于文件对象构建 csv写入对象
csv_writer = csv.writer(f)
# 4. 写入csv文件内容
