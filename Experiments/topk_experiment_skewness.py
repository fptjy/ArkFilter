...
mean = 2 ** 8
k = 200
gama = [0.9, 1.2, 1.5, 1.8, 2.1]
# print(number * mean)
print("实验参数")
print("k=", k)
print("Skewness =", gama)

frequency_rank1 = sampling_zipf(number, number * mean, gama[4])
print("frequency_rank[k]", frequency_rank1[k - 1])
...
for number_k in range(len(gama)):
    ...
    for cishu in range(1):
        ...
        topk_flow = testdata[:k]

        frequency_rank = sampling_zipf(number, number * mean, gama[number_k])

        # print("frequency_rank[k]", frequency_rank[k[number_k] - 1])
        # print(r)
        # print(np.sum(r))
        # print(r[500])
        ...
        for item, count in sorted(counter_Lss.iterateOverThresholdCount(frequency_rank[k]),
                                  key=lambda x: x[1]):
        ...
        precision_Lss = true_number_Lss / k
        ...
        result_AF = AF_Topk_que.maxheap_for_topk(AF, number_of_floors=1, k=k)
        ...
        #     topk_flow_finger.append(AF._get_fingerprint(item))
        #
        # result_AF = AF_Topk_que.minheap_for_topk(AF, number_of_floors=2, k=k)
        #
        # true_number_AF = 0
        # estimate_error_AF = 0
        #
